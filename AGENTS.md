...
- **AXe CLI** ([axe-cli.com](https://www.axe-cli.com/)): `./scripts/build-run-and-axe.sh` oder `./scripts/run-axe-batch.sh`; Schritte in `scripts/axe/*.steps` (Batch: nur Interaktion + `sleep`, kein `screenshot` im Batch). **`run-axe-batch.sh` erzeugt danach immer einen Screenshot** (auch bei Batch-Fehler), Standard `scripts/axe/output/tankradar-launch.png` — eigenen Pfad z. B. `AXE_SCREENSHOT_PATH="$HOME/Desktop/TAN-XX-kurz.png"`. Bei **einzelnen** `axe tap`/`type`/…-Kommandos danach explizit `axe screenshot --udid … --output …` ausführen. Skill: `~/.agents/skills/axe/SKILL.md`. Install: `brew tap cameroncooke/axe && brew install axe`.
