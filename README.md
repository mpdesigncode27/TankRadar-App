...
**Lokal im Simulator testen (ohne dass der Key bei Git verloren geht):** Auf dem Mac eine Datei **`~/.fuelnow/tankerkoenig-api-key`** mit einer Zeile (UUID) anlegen — `./scripts/build-and-run-simulator.sh` reicht den Wert über `SIMCTL_CHILD_TANKERKOENIG_API_KEY` an `xcrun simctl launch` weiter. Alternativ aus Xcode heraus: Run-Scheme → Environment Variables → `TANKERKOENIG_API_KEY` (exakter Name; andere Namen werden nicht gelesen) oder `TANKERKOENIG_API_KEY_FILE` mit absolutem Pfad (siehe Kommentar in `APIKeys.example.swift`). Im DEBUG-Simulator liest die App die Key-Datei zusätzlich automatisch über `SIMULATOR_HOST_HOME`.
